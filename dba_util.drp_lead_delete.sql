...
go
if object_id('dba_util.drp_lead_delete','P') is not null
begin
    set noexec on;
end
go
create procedure dba_util.drp_lead_delete as select 1 as one
go
set noexec off;
go
/*
...
-- Description: Delete Lead conversions (based off dba_util.drp_conversion_delete procedure)

-- 20231218, Andrew Chen, Discovery - Lead
-- =============================================
...
level_mode = 2, Delete
*/
alter procedure [dba_util].[drp_lead_delete]
    @level_id int = 0,
...
begin
    ...
    set @sqlselect = 'select @min_date = min(c.@column_name_date) from [@db_name].@db_schema.@table_name t inner join dbo.conversions c ' +
                     'on t.lead_id = c.lead_id with (nolock) where @column_name_id > 0 and lead_id is not NULL';
